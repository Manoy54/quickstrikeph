...
export default function EventsClient({ events }: { events: EqEvent[] }) {
  ...
  }, [dateFrom, dateTo, events, likelihoodFilter, magFilter, search, sortDir, sortKey, timeFrom, timeTo]);
        ...
        <section className="bg-brand py-6 sm:py-14" aria-labelledby="events-heading">
          <div className="mx-auto max-w-7xl px-3 sm:px-6">
            ...
            <h1 id="events-heading" className="font-display text-xl sm:text-4xl text-white">
              PHIVOLCS Earthquake Events
            </h1>
            <p className="mt-1.5 sm:mt-2 text-xs sm:text-base text-white/80 max-w-2xl">
              Browse, search, and filter PHIVOLCS earthquake information from the last 24 hours.
            </p>
          </div>
        </section>
          ...
          <div className="mx-auto max-w-7xl px-3 sm:px-6 py-3 sm:py-4">
            <div className="flex flex-col gap-3">
              <div className="relative max-w-md">
                <input type="text" placeholder="Search by location, province, or region..." value={search} onChange={(event) => setSearch(event.target.value)} className="w-full pl-4 pr-10 py-2.5 border border-border rounded-lg text-sm focus:outline-none focus:ring-2 focus:ring-brand/30 focus:border-brand" />
                {search ? (
                  <button type="button" onClick={() => setSearch("")} className="absolute right-3 top-1/2 -translate-y-1/2 text-text-muted hover:text-text-primary" aria-label="Clear search">X</button>
                ) : (
        ...
        <div className="mx-auto max-w-7xl px-3 sm:px-6 py-5 sm:py-8">
              ...
              <table className="w-full text-sm" role="table">
                <thead>
                  <tr className="bg-text-secondary text-white text-left">
                    ...
                    <th scope="col" className="px-4 py-3 font-semibold cursor-pointer select-none" onClick={() => toggleSort("date")}>Date &amp; Time{arrow("date")}</th>
                    <th scope="col" className="px-4 py-3 font-semibold">Location</th>
                    <th scope="col" className="px-4 py-3 font-semibold cursor-pointer select-none" onClick={() => toggleSort("magnitude")}>Magnitude{arrow("magnitude")}</th>
                    <th scope="col" className="px-4 py-3 font-semibold">Depth</th>
                    <th scope="col" className="px-4 py-3 font-semibold cursor-pointer select-none" onClick={() => toggleSort("likelihood")}>Likelihood{arrow("likelihood")}</th>
                  ...
                  {pageEvents.length === 0 ? (
                    <tr>
                      <td colSpan={7} className="px-4 py-16 text-center text-text-muted">
                        ...
                        <p className="text-sm">PHIVOLCS has no matching events in the last 24 hours, or the feed is temporarily unavailable.</p>
                      </td>
                    </tr>
                  ) : (
                    pageEvents.map((event, index) => (
                      <tr key={event.id} className={`border-t border-border hover:border-l-4 hover:border-l-brand hover:bg-brand/[0.02] transition-all ${index % 2 === 1 ? "bg-bg-subtle" : ""}`}>
                        ...
                        <td className="px-4 py-3"><span className="text-text-primary">{event.depth}</span><span className="text-text-muted ml-1">km</span></td>
          ...
          <div className="mt-6 flex flex-col sm:flex-row items-center justify-between gap-4 text-sm">
            ...
            <div className="flex items-center gap-1">
              <button type="button" className="px-3 py-1.5 border border-border rounded-md text-text-muted hover:border-brand hover:text-brand transition-colors disabled:opacity-50" disabled={currentPage === 1} onClick={() => setPage((current) => Math.max(1, current - 1))}>Prev</button>
              {Array.from({ length: totalPages }).map((_, index) => {
                const pageNumber = index + 1;
                return (
                  <button key={pageNumber} type="button" onClick={() => setPage(pageNumber)} className={`px-3 py-1.5 rounded-md text-sm font-semibold ${pageNumber === currentPage ? "bg-brand text-white" : "border border-border text-text-muted hover:border-brand hover:text-brand"}`} aria-current={pageNumber === currentPage ? "page" : undefined}>
                    {pageNumber}
                  </button>
                );
              })}
              <button type="button" className="px-3 py-1.5 border border-border rounded-md text-text-muted hover:border-brand hover:text-brand transition-colors disabled:opacity-50" disabled={currentPage === totalPages} onClick={() => setPage((current) => Math.min(totalPages, current + 1))}>Next</button>
            </div>
          </div>
        </div>
      </main>
...
function FilterTag({
  ...
  return (
    <span className="inline-flex items-center gap-1 px-3 py-1 border border-brand text-brand text-xs rounded-full">
      {label}
      <button type="button" onClick={onClear} className="hover:text-high" aria-label={`Remove ${label} filter`}>X</button>
    </span>
  );
}
